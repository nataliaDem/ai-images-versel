...
async function uploadSingleResultToS3({ imageDataUrl, filenameBase, bakeryId }) {
    if (!imageDataUrl) {
    ...
    const key = `ai-images/${bakeryPrefix}${timestamp}-${baseName}.${parsed.extension}`;
...
async function uploadBulkResultsToS3({ bakeryId, items }) {
    ...
    for (const item of items) {
        ...
        const key = `ai-images/${bakeryPrefix}${timestamp}-${productIdPrefix}${baseName}.${parsed.extension}`;
