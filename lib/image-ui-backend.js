...
async function uploadSingleResultToS3({ imageDataUrl, filenameBase }) {
    if (!imageDataUrl) {
    ...
    const key = `ai-images/${timestamp}-${baseName}.${parsed.extension}`;
...
async function uploadBulkResultsToS3({ bakeryId, items }) {
    ...
    for (const item of items) {
        ...
        const key = `ai-images/${timestamp}-${productIdPrefix}${baseName}.${parsed.extension}`;
